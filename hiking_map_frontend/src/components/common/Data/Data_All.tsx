...
export default function Data_All() {
  ...
  const handleExport = async (type: string) => {
    ...
    const res = await fetch(`${baseURL}/trails/export?type=${type}&owner_uuid=${owner?.uuid}`, {
              ...
              <Fragment key={feature.properties?.uuid ?? index}>
                {activeFeatureUuid !== feature.properties?.uuid && (
                  <tr
                    ...
                    className={`${hoverFeatureUuid === feature.properties?.uuid ? styles.hover : ''} ${
                      activeFeatureUuid === feature.properties?.uuid ? styles.active : ''
                    } ${share === 'true' || (!share && feature.properties?.public) ? '' : styles.private}`}
